	...
	: wxPanel(parent, wxID_ANY, pos, size)
{
	...
	radio_on = wxBitmap(mainwin->initpath + "/radio_on-18.png", wxBITMAP_TYPE_PNG);
	radio_off = wxBitmap(mainwin->initpath + "/radio_off-18.png", wxBITMAP_TYPE_PNG);
...
void GraphWindow3::OnPaintGC(wxPaintEvent& WXUNUSED(event))
{
	...
	for(gdisp=0; gdisp<numdisps; gdisp++)
		for(gplot=0; gplot<dispset[gdisp]->numplots; gplot++) {
			...
			if(graph->ytickmode == 2 && graph->ystep > 0) {                     // tick count and spacing for defined linear step size
			...
			for(i=0; i<=ylabels && ylabels>0; i+=1) {
				...
				if(graph->yscalemode == 1 && yfrom > 0) {
					if(graph->ytickmode == 1) yval = yfrom * pow(ylogbase, ylogmax * yval / yto);
			...
			7 - scaled width bars - no spacing between bars unlike histogram

			8 - scatter with sampling

			9 - bar chart with x data

			10 - scatter plot with x data - now subsumed into option for type 2

			11 - mean field plot - custom plot developed for VMN paper
			...
			if(gtype == 3) {                             // spike rate data with optional burst colouring
				...
				//for(i=0; i<(xto-xfrom); i++) {
					...
					*/
					...
					//if(i == ((int)(xto - xfrom + partbin)) - 1) xposend = (int)((i+1)*xrange - xbinoffset);
					//if(i > xto - xfrom + partbin - 2 + 0.05) xposend = (int)((i + 1 - partbin) * xrange);   // last bin, 0.05 to guard against fp error
...
void GraphWindow3::OnPaint(wxPaintEvent &WXUNUSED(event))
{
	...
	for(gdisp=0; gdisp<numdisps; gdisp++)
		for(gplot=0; gplot<dispset[gdisp]->numplots; gplot++) {
			...
			if(graph->yaxis) DrawLine(dc, gc, xbase, ybase, xbase, ybase + yplot);
			if(graph->xaxis) DrawLine(dc, gc, xbase, ybase + yplot, xbase + xplot + xstretch, ybase + yplot);
